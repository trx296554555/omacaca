...
watch(
	...
	() => {
		getData = dataPromise.getGsMmData
		const $grid = xGrid.value
		$grid.commitProxy('query')
		// @ts-ignore
		gridOptions.exportConfig.filename = 'WGCNA-GSMM-RES-module' + geneSetParamStore.wgcna_module
		gridOptions.columns = getTableColumnsConfig()
	}
)

let getData = dataPromise.getGsMmData
// 搜索基因ID
...
function getTableColumnsConfig(): any[] {
	const columns = [
		{
			title: 'No',
			type: 'seq',
		...
		{
			field: `mm_${geneSetParamStore.wgcna_module}`,
			title: `mm_${geneSetParamStore.wgcna_module}`,
			width: '18%',
			sortable: true,
			visible: true,
			formatter: ({ cellValue }) => {
		...
		{
			field: `p_mm_${geneSetParamStore.wgcna_module}`,
			title: `p_mm`,
			sortable: true,
			visible: true,
			formatter: ({ cellValue }) => {
				return cellValue.toExponential(2)
			},
		},
		{
			field: `gs_${geneSetParamStore.wgcna_trait}`,
			title: `gs_${geneSetParamStore.wgcna_trait}`,
			sortable: true,
			visible: true,
			formatter: ({ cellValue }) => {
				return cellValue.toFixed(4)
		...
		{
			field: `p_gs_${geneSetParamStore.wgcna_trait}`,
			title: `p_gs`,
			sortable: true,
			visible: true,
			formatter: ({ cellValue }) => {
...
const gridOptions = reactive<VxeGridProps>({
	...
	exportConfig: {
		type: 'csv',
		filename: 'WGCNA-GSMM-RES-module' + geneSetParamStore.wgcna_module,
		mode: 'all',
		modes: ['all', 'current'],
	},
})
