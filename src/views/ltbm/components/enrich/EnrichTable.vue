...
watch(
	...
	() => {
		...
		// 设置选中行数据用于显示Info
		getData.then((res) => dataPromise.ItemStoreSet(res.data[1]))
		const $grid = xGrid.value
		$grid.commitProxy('query')
		// @ts-ignore
		gridOptions.exportConfig.filename =
			'Enrichment-Res-' + dataPromise.titleSuffix + '-' + switchOption.rereMethodRadio
	}
)
watch(
...
const gridOptions = reactive<VxeGridProps>({
	...
	exportConfig: {
		type: 'csv',
		filename: 'Enrichment-Res-' + dataPromise.titleSuffix + '-' + switchOption.rereMethodRadio,
		mode: 'all',
		modes: ['all', 'current'],
	},
}) // 结束表格配置
